with open("humidity_csv.csv") as f:
	...
	fm=open("data1.json","w")			
	for i in range(0,min(len(in_arr),len(ex_arr))):
		fp.write("db.inhale_exhale.insert({ date:\'"+date[i]+"\',_id:\'"+time[i]+"\',temperature:"+temp[i]+",inhale_time:"+str(in_arr[i])+",exhale_time:"+str(ex_arr[i])+"})\n")
		fm.write("{ \"date\":\""+date[i]+"\",\"_id\":\""+time[i]+"\",\"temperature\":"+temp[i]+",\"inhale_time\":"+str(in_arr[i])+",\"exhale_time\":"+str(ex_arr[i])+"}\n")
	fp.close()
	fm.close()
